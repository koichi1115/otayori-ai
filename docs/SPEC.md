...
| LINE通知のトリガー | **アプリ内DB**を参照（ぷりかん！登録分のみ確実にフィルタ可能） |
...
- 「📄 ぷりかん！登録」のマーカーを含めることで、カレンダー上でぷりかん！経由の登録を識別可能
...
| シナリオ | 概要 |
|---------|------|
| 保護者会の音声補完 | EchoNoteで録音した保護者会・面談の内容から、ぷりかん！と同じ形式でTODO/イベントを抽出・統合 |
| Share Extension | iOSの共有機能を使い、ぷりかん！の要約をEchoNoteに送信、またはその逆 |
